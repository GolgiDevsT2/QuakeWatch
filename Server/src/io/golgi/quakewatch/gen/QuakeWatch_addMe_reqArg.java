...
public class QuakeWatch_addMe_reqArg
{
    ...
    private QuakeFilter filter;

    public QuakeFilter getFilter(){
        return filter;
    }
    public boolean filterIsSet(){
        return filterIsSet;
    }
    public void setFilter(QuakeFilter filter){
        this.filter = filter;
    ...
    private void deserialise(GolgiPayload payload){
        if(!isCorrupt() && payload.containsNestedKey("1")){
            QuakeFilter inst = new QuakeFilter(payload.getNested("1"));
            setFilter(inst);
    ...
    public QuakeWatch_addMe_reqArg(){
        this(true);
    }

    public QuakeWatch_addMe_reqArg(boolean isSetDefault){
        super();
        filterIsSet = isSetDefault;
        filter = new QuakeFilter(isSetDefault);
    }

    public QuakeWatch_addMe_reqArg(GolgiPayload payload){
        this(false);
        deserialise(payload);
    }

    public QuakeWatch_addMe_reqArg(String payload){
